...
- Исключение допускается только для development-only режима `localhost.noNginx: true` в `development.config.json`: если package-local env содержит `VAR_HTTP_ENABLE_PREFLIGHT=true`, `httpServer` может отвечать на browser preflight `OPTIONS` самостоятельно.
...
Исключение — режим `localhost.noNginx: true`, где эти headers отправляет `httpServer` при `VAR_HTTP_ENABLE_PREFLIGHT=true`.
...
Для стандартного init-flow root runner генерирует `VAR_HTTP_PUBLIC_USER_COOKIE_DOMAIN=.gtrktuva.local`, потому что `VAR_HTTP_ORIGIN` и `VUE_APP_BASE_URL` по умолчанию используют development hostnames `test.gtrktuva.local` и `testapi.gtrktuva.local`.
Исключение допускается только для development-only режима `localhost.noNginx: true`: для `localhost` разрешены host-only cookies через `VAR_HTTP_ALLOW_HOST_ONLY_COOKIES=true`.
...
При успешном login authorization gateway должен создать запись в `user_sessions` и записать `sessionUid` в JWT.
`uid` пользователя остается основным identity для существующей authorization logic; `sessionUid` используется для управления устройствами и отдельными browser/device-сессиями.
Каждый login должен логироваться как важное application-событие с IP, user-agent, device type, OS и browser.
Парсинг user-agent выполняется без внешней зависимости и дает диагностическую, а не security-critical классификацию устройства.

Authorization gateway должен предоставлять защищенные endpoints для:

- просмотра активных сессий текущего пользователя;
- отзыва конкретной сессии;
- выхода со всех остальных устройств.

Отзыв текущей сессии должен очищать authorization cookies.
Публичная user cookie не должна содержать `sessionUid`; source of truth остается защищенный JWT и таблица `user_sessions`.
...
HTTP-запросы `GET`, `POST`, `PATCH` и `DELETE` логируются в console output.
В `log_records` обычные `GET` requests не сохраняются; gateway request log сохраняется для state-changing методов `POST`, `PATCH` и `DELETE`, ошибок и других важных событий.
...
При `VAR_APP_LOG_LEVEL=debug` события подключения и отключения WebSocket-клиентов логируются для диагностики с `userId`, `sessionUid`, socket id и user-agent.
В `log_records` рядовые WebSocket connect/disconnect события не сохраняются, чтобы не засорять журнал при refresh, reconnect и навигации пользователя.
