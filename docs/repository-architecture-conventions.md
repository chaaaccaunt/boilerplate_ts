...
```text
services/<service-name>/database-grants.json
gateways/<gateway-name>/database-grants.json
```

Этот файл является источником runtime database grants для development localhost setup.
Root runner должен завершаться ошибкой, если для backend-сервиса или gateway отсутствует `database-grants.json`.
Если package не обращается к БД напрямую, файл должен существовать с пустым списком прав:
...
Ручное редактирование `VAR_DB_RUNTIME_GRANTS` для localhost flow не требуется: `index.js` собирает значение из `database-grants.json`.
...
При добавлении нового backend-сервиса, gateway, frontend package, нового стандартного package-local script или нового режима запуска/сборки агент обязан проверить корневой `index.js` и при необходимости обновить root orchestration.
Если новый package укладывается в существующие workspace globs `services/*`, `gateways/*` или `monolith` и использует уже поддержанные стандартные scripts, менять `index.js` не нужно, но это должно быть осознанно проверено.
Если появляется новый тип package, новый target kind, новый общий script или новая команда orchestration, `index.js` должен быть обновлен в той же задаче.
...
Root runner может поддерживать отдельный development-only режим быстрого запуска без nginx, например `localhost noNginx`.
