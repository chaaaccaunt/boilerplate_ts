...
- запрет удаления последнего суперадминистратора;
- сохранение пароля в виде bcrypt hash при создании;
- разделение доступа к пользователям по правам `users.read`, `users.create`, `users.update`, `users.delete`;
- разделение доступа к ролям по правам `roles.read`, `roles.create`, `roles.update`, `roles.delete`, `roles.permissions.manage`;
...
- пользователь с правом `system.metrics.read` или ролью `superadministrator` открывает страницу состояния системы;
