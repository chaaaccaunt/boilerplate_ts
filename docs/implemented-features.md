...
## Уведомления, MAX и 2FA

Статус: реализовано.

- Внутренние уведомления хранятся в `notifications`, доставляются адресно через WebSocket и открываются отдельной кнопкой-колокольчиком.
- Суперадминистратор задаёт токен и запускает MAX-бота через настройки без перезапуска сервиса; токен хранится только на backend и не возвращается клиенту.
- MAX связывается одноразовой deep link из настроек; после привязки уведомления дублируются ботом.
- При включённой 2FA сессия создаётся только после проверки пароля и одноразового кода из MAX.
- Токен бота доступен только `notifications` service, коды хранятся как SHA-256 с TTL, лимитом попыток и одноразовым использованием.
- Подключение к MAX API проверяет CA bundle и TLS-цепочку до проверки токена; причина ошибки записывается в application log без раскрытия токена.
...
- изменение прав пользовательской роли;
- защита системной роли `superadministrator` от переименования и удаления;
- проверка уникальности логина;
- soft delete пользователя;
- ровно один суперадминистратор и передача его прав выбранному пользователю через отдельное модальное окно;
- отзыв сессий прежнего и нового владельцев роли после передачи для обновления JWT claims;
- сохранение пароля в виде bcrypt hash при создании;
- просмотр пользователей и ролей для любого авторизованного пользователя;
- управление пользователями по праву `users.manage`;
- управление ролями по праву `roles.manage`;
- fallback-доступ к управлению пользователями и ролями через роль `superadministrator`.
...
Статус: реализовано как самостоятельный HTTP-файлообменник.

Покрытый сценарий:

- отдельный экран файлов;
- обзор пользователей в виде папок по умолчанию;
- отдельный route `/files/my` для личных файлов;
- просмотр всех доступных файлов и папок выбранного пользователя;
- загрузка файла;
...
- ленивая генерация preview proxy для ранее загруженных видео при первом запросе превью;
- копирование HTTP-ссылки на файл;
- публичная видимость файлов и папок для всех авторизованных пользователей;
- скрытие отдельного файла владельцем;
- скрытие папки владельцем вместе со всем содержимым;
- доступ суперадминистратора ко всем файлам и папкам;
- редактирование описания своего файла;
- переименование своей папки;
- удаление своей пустой папки;
...
- пользователь с правом `system.read` или ролью `superadministrator` открывает страницу состояния системы и журналы;
