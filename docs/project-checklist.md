...
- Если задача меняла shared contracts или DTO:
  - list/search/summary endpoints не должны возвращать тяжелые поля, которые не используются в списочном UI: HTML, JSON-контент, бинарные данные, большие вложенные массивы;
  - для сокращенного представления нужно создавать отдельный `*ListItemDto`, `*SummaryDto` или другой domain-specific DTO, а полный DTO оставлять для detail/get/update сценариев;
  - обязательные поля DTO нельзя заполнять фиктивными fallback-значениями вроде `[]`, `""` или `null`, если данные реально не были загружены и такое значение может быть воспринято как истинное состояние;
  - если DTO пересекает frontend/backend boundary, проверить все места чтения и записи этого DTO на frontend, gateway, service и shared state.
- Если задача меняла права доступа, route meta, navigation или backend access checks:
  - frontend route meta, sidebar/navigation visibility и backend `accessPermissions` должны требовать согласованный набор прав и ролей;
  - route с `allowedPermissions` не должен становиться доступным всем авторизованным пользователям при отсутствии `allowedRoles`;
  - пользователь, которому показан пункт навигации, должен иметь право открыть default child route этого пункта без ожидаемого 403;
  - fallback на роль `superadministrator` должен быть явным, а не следствием пустой policy-конфигурации.
- Если задача меняла list endpoints, проверки доступа к спискам или работу с иерархиями:
  - нельзя добавлять N+1 DB-запросы в list endpoints;
  - проверки ancestor/parent access должны быть батчевыми, предзагруженными, выраженными в query или заменены на безопасную локальную проверку, если текущий folder context уже гарантирует доступ;
  - приватность родительских сущностей должна проверяться при прямом доступе к вложенному объекту, download/detail/update/delete сценариях.
- Если задача меняла работу с файлами, архивами или большими payload:
  - нельзя читать большие файлы, архивы или наборы пользовательских файлов целиком в память, если сценарий можно реализовать потоково или последовательной записью на диск;
  - лимиты размера должны проверяться до тяжелой операции и не должны скрывать кратное потребление памяти;
  - временные файлы должны иметь cleanup-сценарий и проверку доступа владельца.
- Если задача меняла frontend bulk-операции:
  - нельзя запускать неограниченный `Promise.all` по массиву, размер которого зависит от пользовательского выбора;
  - нужно использовать backend bulk endpoint или frontend concurrency limit;
  - после bulk-операции должен быть один понятный refresh состояния, без каскада лишних reload от каждого realtime-события, если это можно контролировать.
- Если задача меняла realtime:
  - подписка должна возвращать или хранить точечный unsubscribe конкретного handler;
  - запрещено снимать все обработчики события через `off(eventName)`, если событие теоретически может слушать больше одного компонента;
  - глобальные realtime-события должны жить на уровне authenticated layout/store, локальные события допустимы во view и обязаны отписываться при unmount;
  - имена realtime-событий должны быть синхронизированы между shared contracts, backend emit и frontend client type union.
- Если задача меняла frontend UI:
  - продуктовый UI не должен использовать `window.prompt`, `window.confirm` или `window.alert`; нужно использовать компонентные modal/dialog;
  - кнопки и действия должны использовать существующие UI-паттерны и icon-кнопки, если они уже применяются в этой области;
  - пользовательские сообщения должны быть на русском языке.
- Если задача меняла production/dev config, env-copy, build или runner:
...
- Если задача меняла обработку ошибок backend/gateway:
  ...
  - gateway-level validation должна отбрасывать явно некорректные query/body параметры до internal service hop.
