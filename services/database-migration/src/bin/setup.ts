...
setupService.setup()
  .then(() => {
    logger.info("База данных и пользователь сервиса настроены")
  })
  .catch((error) => {
    logger.error("Не удалось настроить базу данных и пользователя сервиса", { error })
    process.exit(1)
  })
...
function getRuntimeUsers(): iDatabaseMigration.RuntimeUserConfig[] {
  const value = process.env.VAR_DB_RUNTIME_GRANTS

  if (!value || value === "УкажитеЗначение") return []
